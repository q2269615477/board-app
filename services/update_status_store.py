...
def is_today_updated(status_file: Optional[Path] = None) -> bool:
    ...
    today = datetime.now().strftime('%Y-%m-%d')
    if status.get('today') != today:
        return False
    daily = status.get('daily_update')
    if isinstance(daily, dict) and daily.get('date') == today:
        return daily.get('status') == 'complete'
    # Backward compatibility for status files created before daily_update
    # carried an explicit completion state.
    return True
