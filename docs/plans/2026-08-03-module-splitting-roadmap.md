...
状态：切片 3.1（坐标几何）、3.2.1（交易覆盖层）、3.2.2（订单栏与持仓汇总框）和 3.3.1（选择/预设选点交互 session）已完成；3.3.2（拖拽与面板生命周期）待实施。
...
- `replay-trade-overlay-renderer.js`：订单栏行与持仓汇总框的被动 SVG 构造也已抽取；订单点击、键盘处理、汇总展开与拖拽仍由 UI 控制层绑定。
- `replay-trade-interaction-controller.js`：3.3.1 已抽取普通 B/S 选择与预设水平选点的短生命周期绑定（几何、交易命令和面板 DOM 仍由 UI 注入）；3.3.2 再抽取拖拽目标与面板生命周期。
