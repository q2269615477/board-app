...
def test_symbol_router_preserves_eastmoney_all_a_code():
    out = _run_js_with_mock(
        ["symbol-router.js"],
        """
const imported = window.SymbolRouter.importSymbol({
  code: '800000', name: '东方财富全A', type: 'index'
});
process.stdout.write(JSON.stringify(imported));
"""
    )
    data = json.loads(out)
    assert data["code"] == "800000"
