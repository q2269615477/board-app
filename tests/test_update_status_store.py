...
class TestTodayUpdated:
    ...
    def test_mark_then_updated(self, tmp_path):
        ...
        assert is_today_updated(sf)
        status = load_status(sf)
        assert status['daily_update']['status'] == 'complete'

    def test_pending_retry_overrides_legacy_today_marker(self, tmp_path):
        sf = tmp_path / 'status.json'
...
class TestIndexStaleControlFlow:
    ...
    def test_empty_database_bootstraps_kline_schema(self, tmp_path):
        import data_update_manager as manager

        db_path = tmp_path / 'kline.db'
        with patch.object(manager, '_LEDGER_DB', str(db_path)), \
        ...
        assert {'kline', 'kline_meta'} <= tables
        assert result['failed'] == 1
        assert result['completion_ready'] is False

    def test_stock_update_bootstraps_empty_database(self, tmp_path):
        import data_update_manager as manager

        db_path = tmp_path / 'stock-kline.db'
        with patch.object(manager, '_LEDGER_DB', str(db_path)), \
             patch.object(manager, 'is_qmt_daily_done', return_value=False), \
             patch.object(manager, '_qmt_http_available', return_value=True):
            result = manager.qmt_update_all_stocks()
        ...
        assert {'kline', 'kline_meta', 'stock_ledger'} <= tables
        assert result['skipped'] is True
        assert result['completion_ready'] is False

    def test_period_materialization_bootstraps_empty_database(self, tmp_path):
        import data_update_manager as manager

        db_path = tmp_path / 'period-kline.db'
        with patch.object(manager, '_LEDGER_DB', str(db_path)), \
             patch.object(manager, 'PREWARM_TARGETS', []):
            result = manager.materialize_higher_periods()
    ...
    def _run_index_update(
        self, tmp_path, local_max, update_status_side_effect=None, qmt_rows=None
    ):
        import data_update_manager as manager
        ...
        with patch.object(manager, 'datetime', fixed_datetime), \
             ...
             patch.object(manager, 'BOARD_ONLY_PREWARM', frozenset()), \
             patch.object(manager, '_qmt_connect', return_value=True), \
             patch.object(manager, 'fetch_qmt_kline', return_value=qmt_rows or []), \
             patch.object(manager, '_tushare_fallback_single_index', return_value=0), \
             patch.object(manager, '_update_status', side_effect=update_status_side_effect) as mocked_update:
            result = manager.update_all_indices_qmt(max_retries=1)
        return result, mocked_update

    def test_yesterday_cannot_complete_target_day_settlement(self, tmp_path):
        captured = {}
        ...
        result, mocked_update = self._run_index_update(
            tmp_path, '2026-07-30', update_status_side_effect=capture_update
        )
        ...
        item = captured['status']['indices']['sh000001']
        assert item['status'] == 'stale_no_source'
        assert item['target_date'] == '20260731'

    def test_target_date_is_success_and_is_recorded(self, tmp_path):
        captured = {}
        ...
        result, mocked_update = self._run_index_update(
            tmp_path, '2026-07-31', update_status_side_effect=capture_update
        )
        ...
        assert result['completion_ready'] is True
        mocked_update.assert_called_once()
        assert captured['status']['indices']['sh000001']['status'] == 'success'
    ...
    def test_nonempty_but_stale_qmt_rows_do_not_complete(self, tmp_path):
        captured = {}
        ...
        result, _ = self._run_index_update(
            tmp_path,
            '2026-07-29',
            update_status_side_effect=capture_update,
            qmt_rows=[stale_row],
        )
        assert result['success'] == 0
        assert result['failed'] == 1
        ...
        item = captured['status']['indices']['sh000001']
        assert item['status'] == 'stale_no_source'
        assert item['local_max'] == '20260730'
