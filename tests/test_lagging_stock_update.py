...
class TestClassifyAndBuild:
    def test_classify_date_lag_sparse_up_to_date(self):
        ...
        assert dum.classify_stock_daily_status('2026-07-27', 50, TARGET_NORM) == 'sparse'
...
class TestQmtUpdateLaggingOnly:
    def _patch_common(self, monkeypatch, db_path, batch_recorder):
        ...
        mock_client.active_channel = 'test'
        mock_client.probe_history_freshness = lambda **kw: {'fresh': True, 'channel': 'test'}

        def fake_batch(codes, **kwargs):
            batch_recorder.extend(list(codes or []))
            return {}

        mock_client.get_daily_batch = fake_batch
        monkeypatch.setattr('data.qmt_client.get_qmt_client', lambda: mock_client)
        monkeypatch.setattr(
            'data.qmt_client.is_mini_data_ready', lambda: True
        )
        # freshness path inside qmt_update uses get_qmt_client from data.qmt_client
        return mock_client
