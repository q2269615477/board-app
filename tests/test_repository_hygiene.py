...
def test_ignored_files_are_not_tracked():
    """Files excluded by .gitignore must not remain in the Git index."""
    out = subprocess.check_output(
        ["git", "-c", "core.quotePath=false", "ls-files", "-ci", "--exclude-standard", "-z"],
        cwd=ROOT,
    )
    offenders = [
        p.decode("utf-8", errors="replace").strip().replace("\\", "/")
    ...
    assert offenders == [], f"Ignored files still tracked: {offenders}"
