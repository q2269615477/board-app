...
注意：11:30 冻结机制已移除，改为实时模式（is_frozen 始终返回 False）。
"""
import sys
import os
import time
...
class TestEnsureSnapshot:
    @patch('services.board_snapshot.datetime')
    @patch('requests.Session', create=True)
    def test_real_time_mode_afternoon(self, mock_session_cls, mock_dt):
        """实时模式：11:30 后 ensure_snapshot 仍 capture，is_frozen() 始终 False"""
        # 模拟时间：周一 11:35（午休）—— 不再冻结，改为实时模式
