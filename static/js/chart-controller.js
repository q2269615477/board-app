...
(function (global) {
  ...
  function _replayController() {
    return global.BarReplayController || window.BarReplayController;
  }

  function _replayIsActive() {
    var replay = _replayController();
    return !!(replay && typeof replay.isActive === 'function' && replay.isActive());
  }

  function _exitReplayForContextChange() {
    var replay = _replayController();
    if (replay && typeof replay.exit === 'function' && _replayIsActive()) {
      replay.exit({ restore: false, silent: true, reason: 'context-change' });
    }
  }

  function _resetVerticalPanForContextChange() {
    var pan = global.ChartVerticalPanController || window.ChartVerticalPanController;
    if (pan && typeof pan.reset === 'function') pan.reset({ silent: true });
  }
  ...
  function applySymbol(sel) {
    ...
    _exitReplayForContextChange();
    _resetVerticalPanForContextChange();
  ...
  function refreshCurrent(meta) {
    if (_replayIsActive()) {
      var replay = _replayController();
      if (replay && typeof replay.markRefreshPending === 'function') {
        replay.markRefreshPending(meta || {});
      }
      return false;
    }
