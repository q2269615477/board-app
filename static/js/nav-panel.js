...
async function loadClassification() {
  ...
  async function attemptLoad() {
    try {
      ...
      // 分类数据到达后立即渲染，标注计数不得阻塞左栏首屏。
      renderNav();
      refreshAnnCounts(false).then(_patchAnnCountsInNav);
    } catch(e){
...
async function refreshAnnCounts(shouldRender) {
  if (shouldRender == null) shouldRender = true;
  try {
    var r = await fetch(API + '/api/annotations/counts');
    var j = await r.json();
    _annCounts = (j && j.data) || {};
    if (shouldRender) renderNav();
  } catch (e) {}
}

function _patchAnnCountsInNav() {
  document.querySelectorAll('.board-item[data-code]').forEach(function(item) {
    var main = item.querySelector('.board-main');
    if (!main) return;
    var count = _annCounts[item.dataset.code];
    var badge = main.querySelector('.board-ann');
    if (!count) {
    ...
    if (!badge) {
      ...
      badge.className = 'board-ann';
      main.appendChild(badge);
    }
    badge.title = '已画 ' + count + ' 条支撑/阻力位';
...
let _navSearchHistoryOpen = false;
let _navSearchHistoryIndex = -1;
let _navSearchHistoryHydrationRequested = false;

// search-history-store.js is optional at runtime because the legacy entry
// page does not load it. Keep a compatible localStorage fallback so the nav
// remains usable and can share data with search-panel.js in that case.
function _getNavSearchHistoryStore(){
...
function _renderNavSearchHistoryPanel(){
  var history = [];
  try { history = _getNavSearchHistoryStore().list().slice(0,5); } catch(_) {}
  if(!history.length) return '';
  var visible = _navSearchHistoryOpen && !_navSearchQuery;
  var h = '<div id="nav-search-history" class="search-results" role="listbox" style="position:absolute;left:10px;right:10px;top:62px;max-height:190px;overflow-y:auto;z-index:20;display:' + (visible ? 'block' : 'none') + ';background:inherit;border:1px solid currentColor;box-shadow:0 8px 20px rgba(0,0,0,.25);">';
  h += '<div class="search-history-header" style="padding:6px 10px;font-size:11px;opacity:.7;">最近搜索</div>';
  history.forEach(function(item, index){
    var label = item.name || item.value || item.code || '';
    var detail = item.display_code || item.code || item.value || '';
    h += '<div class="search-item nav-search-history-item' + (index === _navSearchHistoryIndex ? ' selected' : '') + '" role="option" data-nav-history="' + index + '" title="恢复搜索：' + escAttr(item.value || label) + '">';
    h += '<span class="search-name" style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escHtml(label) + '</span>';
    h += '<span class="search-code" style="margin-left:auto;opacity:.75;">' + escHtml(detail) + '</span></div>';
  });
  return h + '</div>';
}

function _updateNavSearchHistoryVisibility(){
  var panel = document.getElementById('nav-search-history');
  if(!panel) return;
  var history = [];
  try { history = _getNavSearchHistoryStore().list().slice(0,5); } catch(_) {}
  panel.innerHTML = _renderNavSearchHistoryPanel().replace(/^<div[^>]*>|<\/div>$/g, '');
  panel.style.display = (_navSearchHistoryOpen && !_navSearchQuery && history.length) ? 'block' : 'none';
  panel.querySelectorAll('[data-nav-history]').forEach(function(item){
    item.classList.toggle('selected', Number(item.dataset.navHistory) === _navSearchHistoryIndex);
  });
}

function showNavSearchHistory(){
  _navSearchHistoryOpen = !_navSearchQuery;
  _navSearchHistoryIndex = _navSearchHistoryOpen ? 0 : -1;
  _updateNavSearchHistoryVisibility();
  if(!_navSearchHistoryHydrationRequested && typeof window.ensureBoardSearchHistory === 'function'){
    _navSearchHistoryHydrationRequested = true;
    window.ensureBoardSearchHistory().then(function(){
      if(!_navSearchQuery && store.categoryData) renderNav();
    }).catch(function(){});
  }
}

if (typeof window !== 'undefined') {
  window.addEventListener('board-search-history-ready', function(){
    if (store.categoryData) renderNav();
  });
}

// renderNav() rebuilds the search input whenever the navigation tree changes.
// Delegate the open action so a newly-created input always keeps history access.
if (typeof document !== 'undefined' && !document.__boardNavSearchHistoryBound) {
  document.__boardNavSearchHistoryBound = true;
  document.addEventListener('pointerdown', function(event){
    var input = event && event.target && event.target.closest
      ? event.target.closest('#nav-search-input')
      : null;
    if (!input) return;
    _navSearchQuery = String(input.value || '').trim().toLowerCase();
    _navSearchHistoryOpen = !_navSearchQuery;
    _navSearchHistoryIndex = _navSearchHistoryOpen ? 0 : -1;
    setTimeout(_updateNavSearchHistoryVisibility, 0);
  });
  document.addEventListener('focusin', function(event){
    if (!event || !event.target || event.target.id !== 'nav-search-input') return;
    _navSearchQuery = String(event.target.value || '').trim().toLowerCase();
    _navSearchHistoryOpen = !_navSearchQuery;
    _navSearchHistoryIndex = _navSearchHistoryOpen ? 0 : -1;
    setTimeout(_updateNavSearchHistoryVisibility, 0);
  });
...
function handleNavSearchKeydown(event){
  if(!event) return;
  var panel = document.getElementById('nav-search-history');
  var items = panel ? panel.querySelectorAll('[data-nav-history]') : [];
  if(!_navSearchHistoryOpen || _navSearchQuery || !items.length) return;
  if(event.key === 'ArrowDown'){
    event.preventDefault();
    _navSearchHistoryIndex = Math.min(items.length - 1, _navSearchHistoryIndex < 0 ? 0 : _navSearchHistoryIndex + 1);
  }else if(event.key === 'ArrowUp'){
    event.preventDefault();
    _navSearchHistoryIndex = Math.max(0, _navSearchHistoryIndex <= 0 ? 0 : _navSearchHistoryIndex - 1);
  }else if(event.key === 'Enter'){
    event.preventDefault();
    selectNavSearchHistory(_navSearchHistoryIndex < 0 ? 0 : _navSearchHistoryIndex);
    return;
  }else if(event.key === 'Escape'){
    event.preventDefault();
    _navSearchHistoryOpen = false;
    _navSearchHistoryIndex = -1;
    _updateNavSearchHistoryVisibility();
    return;
  }else{
    return;
...
function selectNavSearchHistory(index){
  var history = [];
  try { history = _getNavSearchHistoryStore().list().slice(0,5); } catch(_) {}
  var item = history[index];
  if(!item) return;
  var value = String(item.value || item.name || item.code || '').trim();
  if(!value) return;
  try { _getNavSearchHistoryStore().add(item); } catch(_) {}
  _navSearchQuery = value.toLowerCase();
  _navSearchHistoryOpen = false;
  _navSearchHistoryIndex = -1;
  renderNav();
...
function _rememberNavSearchSelection(name, code, type){
  var value = String(_navSearchQuery || '').trim();
  if(!value) return;
  var item = { code: code, value: value, name: name, type: type, time: Date.now() };
  try {
    _getNavSearchHistoryStore().add(item);
  } catch(_) {}
  if(typeof window.recordBoardSearchHistory === 'function') window.recordBoardSearchHistory(item);
}
...
function renderNav() {
  ...
  let h = '';
  // 顶部卡片搜索框
  h += '<div class="nav-header-box" style="position:relative;">';
  h += '  <div class="nav-header-title"><span>产业链板块 (' + totalBoards + '个)</span></div>';
  h += '  <input id="nav-search-input" class="nav-search-input" autocomplete="off" value="' + escAttr(_navSearchQuery) + '" placeholder="🔍 搜板块 / 拼音 / 代码 / tag / #tag" oninput="filterNavTree(this.value)" onkeydown="handleNavSearchKeydown(event)">';
  h += _renderNavSearchHistoryPanel();
  h += '</div>';
  ...
  store.categoryData.forEach(cat => {
    ...
    const navHasFilter = !!(_navSearchQuery || _navActiveTags.size || _navFilterType !== 'all');
    const hasCatState = store._expandedCats && Object.prototype.hasOwnProperty.call(store._expandedCats, cat.name);
    const isCatExpanded = hasCatState ? store._expandedCats[cat.name] === true : navHasFilter;
    ...
    if (cat.subcategories && cat.subcategories.length > 0) {
      cat.subcategories.forEach((sub, sidx) => {
        ...
        const hasSubState = store._expandedCats && Object.prototype.hasOwnProperty.call(store._expandedCats, subId);
        const isSubExpanded = hasSubState ? store._expandedCats[subId] === true : navHasFilter;
  ...
  var navHistoryPanel = p.querySelector('#nav-search-history');
  if (navHistoryPanel) {
    navHistoryPanel.addEventListener('mousedown', function(event){
      var item = event.target.closest('[data-nav-history]');
      if (!item) return;
      event.preventDefault();
      selectNavSearchHistory(Number(item.dataset.navHistory));
    });
  }
  var navSearchInput = p.querySelector('#nav-search-input');
  if (navSearchInput) {
    navSearchInput.addEventListener('focus', showNavSearchHistory);
    navSearchInput.addEventListener('mousedown', showNavSearchHistory);
  }
  _updateNavSearchHistoryVisibility();

  // 分类/搜索会重建列表节点；把已取得的行情缓存重新应用到新节点，
  // 否则切换“行业/概念/指数”后涨跌幅会退回占位符“--”。
  applyBoardChgToDOM();
  ...
  // 指数不属于 /api/board-changes 的板块快照；切换到指数分类后，
  // 按当前列表批量补取实时行情，避免指数项长期停留在“--”。
  loadIndexBoardChanges();
}
...
function selectBoard(name,code,type,ev){
  if (type === 'index' && _isIndexBoardUnavailable(code)) {
  ...
  // 只有在左上搜索框确实有查询词时，点击结果才进入搜索历史；
  // 普通浏览、最近访问和分类点击不会污染搜索历史。
  _rememberNavSearchSelection(name, code, type);
...
function updateChartHeaderChg(name, code, type){
  ...
  if ((isIndex || reqType === 'stock') && !renderedFromIndexBar) {
...
// Pro 就绪后做一次兜底；K 线完成和顶部行情到达都会事件驱动更新。
...
function showConsPanel(name, code, type, ev){
  ...
  const boardType = type === 'industry' ? 'industry' :
    (type === 'index' ? 'index' : 'concept');
  ...
  function renderBody(){
    ...
    _consData.forEach(function(s){
      ...
      const chgCls = chgNum > 0 ? 'up' : (chgNum < 0 ? 'down' : 'flat');
...
let _indexChgLoadPromise = null;
let _indexChgRequested = new Set();
...
function _isBoardQuoteUnavailable(raw){
  return !!(raw && typeof raw === 'object' &&
    (raw.unavailable === true || raw.reason === 'deprecated_no_remote'));
}

function _isIndexBoardUnavailable(code){
  var raw = _boardChgData && (_boardChgData['index:' + code] !== undefined
    ? _boardChgData['index:' + code] : _boardChgData[code]);
  if (_isBoardQuoteUnavailable(raw)) return true;
  var el = document.querySelector('.board-item[data-type="index"][data-code="' + escAttr(code) + '"]');
  return !!(el && el.dataset.unavailable === '1');
}
...
function applyBoardChgToDOM(){
  ...
  document.querySelectorAll('.board-item').forEach(function(el){
    ...
    if (span && _isBoardQuoteUnavailable(chg)) {
      span.className = 'board-chg unavailable';
      span.textContent = '停更';
      span.title = '该指数已停更，暂无可用行情';
...
function loadIndexBoardChanges(force){
  ...
  const codes = Array.from(new Set(rows.filter(function(el){
    const code = el.dataset.code;
    const type = el.dataset.type;
    const cached = _boardChgData && (_boardChgData[type + ':' + code] !== undefined
  ...
  _indexChgLoadPromise = fetch(API + '/api/spot/indices?' + params.toString())
    ...
    .then(function(resp){
      ...
      if (!data || typeof data !== 'object' || !Object.keys(data).length) {
        throw new Error('empty index quote response');
      }
      if (!_boardChgData) _boardChgData = {};
      Object.entries(data).forEach(function(entry){
        const code = entry[0];
        if (_isBoardQuoteUnavailable(entry[1])) {
          _boardChgData[code] = entry[1];
          _boardChgData['index:' + code] = entry[1];
          return;
        }
        const value = _boardChgNumber(entry[1]);
        if (value == null) return;
        _boardChgData[code] = value;
        _boardChgData['index:' + code] = value;
      });
      applyBoardChgToDOM();
    })
    .catch(function(e){
