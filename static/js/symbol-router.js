...
(function (global) {
  ...
  function classify(code) {
    ...
    // A 股指数 (sh/sz 前缀 + 6位数字，且是知名指数代码)
    if (/^(sh|sz|bj)\d{6}$/.test(c)) {
      // 已知 A 股指数前缀
      var idxPrefixes = ['sh0000', 'sh0001', 'sh0003', 'sh0006', 'sh0008', 'sh0009',
                         'sz3990', 'sz3991', 'sz3992', 'sz3993', 'sz3994'];
      for (var i = 0; i < idxPrefixes.length; i++) {
        if (c.indexOf(idxPrefixes[i]) === 0) return 'index';
      }
      // sh/sz：6位纯数字的是个股
      return 'stock';
    }
