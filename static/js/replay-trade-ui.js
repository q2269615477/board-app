(function (global) {
  ...
  function indexFromValue(value, rows) {
    ...
    for (var index = 0; index < rows.length; index += 1) {
      if (timestamp(rows[index].timestamp) === ts) return index;
    }
    return -1;
  }
  ...
  function convertPixelToPrice(event, chart, dom) {
    ...
    if (coordinates.y == null || !chart || typeof chart.convertFromPixel !== 'function') return null;
    var x = coordinates.x;
    if (x == null) {
      var width = dom && (dom.clientWidth || (dom.getBoundingClientRect && dom.getBoundingClientRect().width));
      x = Number(width) > 0 ? Number(width) / 2 : 0;
    }
    ...
    for (var index = 0; index < attempts.length; index += 1) {
      try {
        var converted = chart.convertFromPixel(attempts[index], { paneId: 'candle_pane' });
        var price = convertedPrice(converted);
        if (price != null && price > 0) {
          return { price: price, x: x, y: coordinates.y };
        }
      } catch (e) {}
    }
    return null;
  ...
  function getPointFromEvent(event, chart, dom) {
    ...
    var x = coordinates.x;
    var y = coordinates.y;
    if (index < 0 && chart && typeof chart.convertFromPixel === 'function' && x != null) {
      var converted = null;
      try { converted = chart.convertFromPixel([{ x: x, y: y == null ? 0 : y }], { paneId: 'candle_pane' }); } catch (e) {
        try { converted = chart.convertFromPixel({ x: x, y: y == null ? 0 : y }, { paneId: 'candle_pane' }); } catch (ignore) {}
      }
      var point = Array.isArray(converted) ? converted[0] : converted;
      if (point) index = indexFromValue(point.dataIndex, rows);
    }
    ...
    if (index < 0 && x != null) {
      var width = dom && (dom.clientWidth || (dom.getBoundingClientRect && dom.getBoundingClientRect().width));
      if (width > 0) index = Math.max(0, Math.min(rows.length - 1, Math.round((x / width) * (rows.length - 1))));
    }
  ...
  function convertPrice(index, price, rows, width, height) {
    ...
    if (chart && typeof chart.convertToPixel === 'function') {
      ...
      for (var i = 0; i < attempts.length; i += 1) {
        try {
          var point = chart.convertToPixel(attempts[i], { paneId: 'candle_pane' });
          point = Array.isArray(point) ? point[0] : point;
          if (point && finite(point.x) != null && finite(point.y) != null) return { x: Number(point.x), y: Number(point.y) };
        } catch (e) {}
      }
    }
    var x = rows.length > 1 && finite(index) != null ? (Number(index) / (rows.length - 1)) * width : width / 2;
    var values = rows.map(function (row) { return [finite(row.high), finite(row.low)]; })
      .reduce(function (all, pair) { if (pair[0] != null) all.push(pair[0]); if (pair[1] != null) all.push(pair[1]); return all; }, []);
