...
function applyIdxPrices(dataMap) {
  ...
  Object.entries(dataMap).forEach(([code, v]) => {
    ...
    const priceStr = safeFix(priceNum, code.match(/^HSI$|HSTECH|N225/) ? 0 : 2);
    ...
    // 姣忔鏇存柊瑙﹀彂 3s 鑴夊啿楂樹寒
    el.className = 'price ' + colorClass + ' ' + flashClass;
    setTimeout(() => {
    ...
    if (itemObj) itemObj.price = chgStr + ' (' + priceStr + ')';
...
function _startIdxFallback() {
  if (_idxFallbackTimer) return;
  refreshIdxPrices();
  _idxFallbackTimer = setInterval(refreshIdxPrices, 10000);
}
...
async function refreshIdxPrices() {
  try {
    ...
    const encodedTickers = tickers.replace(/\^/g, '%5E');
    const url = apiBase + '/api/spot/indices' + (tickers ? '?tickers=' + encodedTickers : '');
