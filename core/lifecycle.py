...
class AppContext:
    ...
    def _reload_board_changes(self):
        """优先用 spot API 获取板块涨跌幅，CSV 兜底"""
        result = {}

        # === 优先路径：spot API ===
        try:
            from data import board_api
            ind_spot = board_api.get_industry_spot()
            con_spot = board_api.get_concept_spot()
            ind_spot = ind_spot if isinstance(ind_spot, dict) else {}
            con_spot = con_spot if isinstance(con_spot, dict) else {}
        except Exception as e:
            logger.debug(f"[涨跌幅] spot API 不可用: {e}")
            ind_spot, con_spot = {}, {}

        # spot API 有数据时用 spot 构建缓存
        if ind_spot or con_spot:
            # 先处理 industry：industry 占用 bare key 优先
            for code, info in ind_spot.items():
                chg = info.get('涨跌幅') if isinstance(info, dict) else None
                if chg is None:
                    continue
                result[f'industry:{code}'] = chg
                result[code] = chg  # industry 优先占 bare key

            # 再处理 concept：bare key 仅当 industry 未占用
            for code, info in con_spot.items():
                chg = info.get('涨跌幅') if isinstance(info, dict) else None
                if chg is None:
                    continue
                result[f'concept:{code}'] = chg
            ...
            with self.board_chg_lock:
                ...
                if result:
                    logger.info(f"[涨跌幅] spot API 已加载 {len(result)} 个板块涨跌幅")
            return
        ...
        with self.board_chg_lock:
            self.board_chg_cache = result
            if result:
                logger.info(f"[涨跌幅] CSV 已加载 {len(result)} 个板块涨跌幅")
