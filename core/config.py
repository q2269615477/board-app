...
# 数据目录：支持 BOARD_APP_DATA_DIR 环境变量覆盖（浏览器测试/临时沙箱注入路径），
# 未设置或为空时保持默认 BASE_DIR/data 完全不变
DATA_DIR = Path(os.environ.get('BOARD_APP_DATA_DIR') or str(BASE_DIR / 'data'))
...
# 启动时只探测 QMT 是否可用，不再默认重写 2020 年以来的历史库。
# 历史数据由日更管线或显式维护任务负责；确需兼容旧行为时再开启。
QMT_STARTUP_HISTORY_SYNC = os.environ.get('QMT_STARTUP_HISTORY_SYNC', '0') == '1'
# 首屏请求本身走缓存优先，启动时并发预热全部指数会与 QMT/调度器争抢资源。
STARTUP_PREWARM = os.environ.get('BOARD_APP_STARTUP_PREWARM', '0') == '1'
...
NAV_QMT_TYPES = frozenset({'index', 'stock'})
...
UPDATE_SCHEDULE_TIMES = {
    ...
    'noon_freeze': '11:30',       # 冻结上午最后有效快照
    'afternoon_switch': '13:00',  # 下午开盘切换
    'qmt_settlement': '15:30',    # 指数/个股 QMT 正式日线
    'board_settlement': '17:00',  # Tushare 东财板块正式日线
}
