...
class QmtHttpClient:
    def __init__(
        ...
        if session is not None:
            # 显式传入的 Session 原样使用，不改写其代理/信任配置
            self._session = session
        else:
            # 默认 Session 显式直连：不走代理环境变量，也不影响全局 requests
    ...
    def _circuit_remaining(self) -> float:
        with self._state_lock:
            return max(0.0, self._unavailable_until - time.monotonic())

    def _mark_transport_failure(self) -> None:
        with self._state_lock:
            self._unavailable_until = time.monotonic() + _CIRCUIT_COOLDOWN_SEC
    ...
    def ohlc_batch(
        ...
        # One failed local connection is enough evidence for a short cooldown.
        # Serialize probes so concurrent UI requests do not each wait for the
        # same refused 18080 connection.
        with self._request_lock:
            remaining = self._circuit_remaining()
            if remaining > 0:
                return {
                    "ok": False,
                    "error": "qmt_http_circuit_open",
                    "items": {},
                    "errors": skipped,
                    "channel": "qmt18080",
            ...
            if transport_failed and not merged_items:
