...
def classify_stock_daily_status(max_date_str, row_count, target_norm):
    """分类个股日线数据状态。
...
def build_stock_pending_from_ledger(stocks, max_dates, row_counts, target_norm):
    """根据台账信息构建 pending 列表。
...
def qmt_update_all_stocks(
    ...
    if not canceled and mark_done and limit is None:
        _mark_qmt_daily_done()
    elif not canceled and mark_done and limit is not None and len(pending) < (limit or 0):
        _mark_qmt_daily_done()
    return result
...
def _update_single_board(board_type: str, name: str, code: str):
...
def update_all_today(max_retries: int = 3, cancel_check=None, progress_callback=None) -> dict:
    ...
    if qmt_ok:
        result['stocks'] = qmt_update_all_stocks(max_retries, cancel_check=cancel_check)
